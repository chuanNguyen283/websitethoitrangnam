        ...
        <div class="flex flex-col sm:justify-center sm:ml-10 ">
          <div class="grid grid-cols-2">
            ...
            <div class="text-black font-[1rem] font-medium">
              <p>{{ record.product.price }} VNĐ</p>
              <p>{{ record.quantity }}</p>
              <p class="text-red">
                {{ record.product.price * record.quantity }} VNĐ
              </p>
            </div>
          </div>
        </div>
      ...
      <template slot="footer">
        ...
        <div class="flex sm:justify-between w-1/2 mx-auto my-10 flex-col sm:flex-row">
          <button ref="checkOutCarts" class="bg-green text-white py-2 px-2 text-[1rem] font-medium rounded-sm" disabled @click="showBilling">
            ...
            Tiến hành thanh toán ({{ numberProduct }})
          </button>
  ...
  methods: {
    ...
    async editItem (idCartItem) {
      try {
        ...
        this.$toast.success('Cập nhật giỏ hàng thành công', { timeout: 1500 })
        this.$store.dispatch('dataCart')
        this.loading = false
        // console.log('CartItem: ', cartItem)
      } catch (error) {
        this.$toast.error('Lỗi', { timeout: 1500 })
        // console.log(error)
        this.loading = false
      }
    },
    async deleteCarts () {
      try {
        ...
        this.$store.dispatch('dataCart')
        this.$toast.success('Xóa thành công khỏi giỏ hàng!', { timeout: 1500 })
      } catch (error) {
        console.log(error)
      }
    },
