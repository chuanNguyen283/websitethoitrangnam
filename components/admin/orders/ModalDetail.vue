<template>
  <div>
    <a-modal v-model="visible" :footer="null" title="Chi tiết đơn hàng" width="{980}" class="modal_detail max-h-[50vh] overflow-y-auto">
      <a-table
        :columns="columns"
      ...
      >
        ...
        <template slot="footer">
          <div class="flex flex-col">
            <div>
              <a-form-model-item label="Địa chỉ nhận hàng">
                <a-input
                  v-model="address"
                  disabled
                />
              ...
              <div class="flex sm:w-full sm:flex-row flex-col  sm:space-x-3">
                <a-form-model-item label="Tổng tiền" class="w-full">
                  <a-input
                    v-model="totalPrice"
                    disabled
                    addon-after="VNĐ"
  ...
  methods: {
    ...
    async acceptStatus (orderId) {
      try {
        ...
        this.$toast.success('Thành công!', { timeout: 1500 })
    ...
    async canceled (_id) {
      try {
        ...
        this.$toast.success('Hủy thành công!', { timeout: 1500 })
        window.location.reload()
      } catch (error) {
        this.$toast.error('System Error', { timeout: 1500 })
        console.log(error)
