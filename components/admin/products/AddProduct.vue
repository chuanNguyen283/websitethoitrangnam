<template>
  ...
  >
    <div class="flex flex-col justify-center add_product">
      ...
      <a-form-model-item ref="category" label="Danh Mục" prop="category">
        <a-input
      ...
      <a-form-model-item label="Ảnh">
          ...
          <div class="my-3">
            <label for="inputfile" class="cursor-pointer text-black">
              <span class="font-semibold">
                <i class="fa-regular fa-folder-open text-2xl" /> Chọn Anh
              </span>
              <input
                id="inputfile"
                type="file"
        ...
        <a-form-model-item label="" prop="subject">
        <a-switch v-model="form.subject" :class="form.subject? 'bg-blue' : 'bg-[#ccc]'" />
        <span class="text-black ml-4">{{ form.subject ? 'Phụ Kiện' : 'Thời Trang' }}</span>
      </a-form-model-item>
      </a-form-model-item>
      <div class="grid grid-cols-2 gap-7">
        <a-form-model-item ref="price" label="Gía KM" prop="price">
          <a-input
            v-model="form.price"
            addon-after="VNĐ"
            @blur="
        ...
        <a-form-model-item ref="originalPrice" label="Gía" prop="originalPrice">
          <a-input
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        name: [
          { required: true, message: 'Tên sp', trigger: 'blur' },
          { min: 3, max: 100, message: 'Length should be 3 to 5', trigger: 'blur' }
        ],
        category: [
          { required: true, message: 'Danh mục sp', trigger: 'blur' },
          { min: 3, max: 100, message: 'Length should be 3 to 5', trigger: 'blur' }
        ],
        description: [{ required: true, message: 'Mô tả sp', trigger: 'blur' }],
    ...
    onSubmit () {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          ...
          this.$toast.success('Thêm sản phẩm thành công!', { timeout: 1500 })
          this.form.images = []
        } else {
