      ...
      >
        ...
        <span class="text-black ml-4">{{ form.subject ? 'Phụ Kiện' : 'Thời Trang' }}</span>
          ...
          <div class="grid grid-cols-2 gap-7">
            <a-form-model-item ref="price" label="GIÁ KM" prop="price">
              <a-input
                v-model="form.price"
                addon-after="VNĐ"
                @blur="
            ...
            <a-form-model-item ref="originalPrice" label="GIÁ" prop="originalPrice">
              <a-input
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        name: [
          { required: true, message: 'Trường phải điền!', trigger: 'blur' },
        ...
        description: [{ required: true, message: 'Trường phải điền!', trigger: 'blur' }],
  ...
  methods: {
    ...
    async submitdata (formData, imagesDelete) {
      try {
        ...
        this.$toast.success('Sửa thành công!', { timeout: 1500 })
        setTimeout(() => {
          location.reload()
        }, 1000)
      } catch (error) {
    ...
    async editProduct (formData, imagesDelete) {
      try {
        await this.$api.product.editProduct({ formData, imagesDelete })
        this.$toast.success('Sửa thành công!r', { timeout: 1500 })
      } catch (error) {
