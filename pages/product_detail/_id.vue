    ...
    <div class="px-8 flex sm:flex-row flex-col  gap-6 lg:my-10 my-5">
      ...
      <div class="flex flex-col space-y-5 pl-4">
        ...
        <p class="text-black font-medium text-[1rem]">
          <a-rate v-model="rating" disabled allow-half />  ( {{ product.rating }}/5)
        </p>
        <p class="text-black font-medium text-2xl">
          {{ product.price }} VNĐ <span class="text-orange line-through ml-2">{{ product.price*1.2 }} VNĐ</span>
        </p>
...
export default {
  ...
  methods: {
    async addToCart () {
      if (!this.value) {
        this.$toast.error('Hãy chọn size', { timeout: 1500 })
        ...
        try {
          ...
          this.$toast.success('Thêm thành công', { timeout: 1500 })
        } catch (error) {
          console.log('error')
        }
      } else {
